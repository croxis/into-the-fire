...
func calculate_vectors() -> void:
	...
	var force_forward := transform.basis.z.normalized().dot(spaceship.transform.basis.z)
	var force_right := global_transform.basis.z.dot(spaceship.global_transform.basis.x)
	var force_up := global_transform.basis.z.dot(spaceship.global_transform.basis.y)
	...
	max_force_vector = Vector3(force_right, force_up, force_forward) * max_force * health
	max_torque_vector = Vector3(torque_right, torque_up, torque_forward) * max_force * health
