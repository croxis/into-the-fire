...
var unlaunched_pilots: int = 0
var launched_pilots: int = 0
var desired_pilots: int = 0
...
func _init(p_desired_pilots: int = 0) -> void:
	# Assign any parameters here.
	self.desired_pilots = p_desired_pilots


# Override
func initialize(agent: GdPAIAgent) -> void:
	# Set up initial property values with the agent.
	agent.blackboard.set_property("desired_pilots", desired_pilots)
	agent.blackboard.set_property("unlaunched_pilots", unlaunched_pilots)
